...
-------------------------------------*/
float randomNum::randRangeF(randomNum& prng, float low, float high) {
    LS_DEBUG_ASSERT(low < high);
    return
        low + (
            (float)prng() / (
                (float)std::numeric_limits<unsigned long>::max() / (high-low)
            )
        );
}
...
-------------------------------------*/
int randomNum::randRangeI(randomNum& prng, int low, int high) {
    LS_DEBUG_ASSERT(low < high);
